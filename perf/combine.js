...
var s1 = streamjs.Observable.from(a);
var s2 = streamjs.Observable.from(a);
var s3 = streamjs.Observable.from(a);

suite
  .add('streamjs', function(deferred) {
    runners.runStreamJS(deferred,
      streamjs.combine(s1, s2, s3).map(add3Arr).filter(even));
  }, options)
